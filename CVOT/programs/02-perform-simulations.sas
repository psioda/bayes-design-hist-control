...
%let stratLevels  = 3;
%let fitHazComp   = 1 | 1 | 2;
%let fitHazBreaks = . | . | 1.04;
%let rProb        = 0.5;
%let enrollDist   = rand('beta',1,2)*3;
%let censorDist   = 1000;

** reduction in total sample size;
%let borrowAmt     = 0.000 | 0.125 | 0.250 | 0.375 | 0.500 ; 
...
%put iteration &loop of &nSimSettings.;
 ...
 data start; x = time(); run;
  ...
  data tempsp;
   set sp.Parameter_perturbations;
...
data Results2;
 ...
 by row_idx;
  if a and b;
  drop row_idx parmSet;
  simStudy = simStudy + 1000000*parmset;
run;

data res.sim_results_&dsLabel.;
 set Results2;
run;
