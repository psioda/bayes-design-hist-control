...
This macro requires the following parameters:
  ...
  MACRO VARIABLES REQUIRED:
   ...
   [4] dsFull       = dataset to be used as input for data reduction;
   [5] dsCollapsed  = dataset to be written out after data reduction;
...
proc means data = __tempData__ noprint nway;
  class simstudy riskStratum x: interval;
  var r v;
  output out = &dsCollapsed.(drop=_type_ _freq_) sum=riskTime numEvents;
run;
